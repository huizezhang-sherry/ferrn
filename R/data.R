#' Simulated data
#'
#' The \code{boa} data is simulated using different Gaussian mixtures with varied centres and weights,
#' whilst those prefixed \code{holes_*} are data objects summarising the optimisation paths
#' generated by the tourr package (see format for the code of simulation). The \code{boa} data can be used to
#' explore performance of different optimisation routines in the tourr, and the \code{holes_*} is used to
#' test out the diagnostic plots provided in this package.
#'
#' Boa is given as the name of the dataset because the density plot of each variable in the data looks like boa constrictors swallowing multiple French baguettes, rather than elephants, as per the little prince.
#'
#' \code{holes} indicates the usage of holes index, \code{1d} or \code{2d} represent if the five variable \code{boa5} or the six variable \code{boa6} is used
#' The third chunk of the name indicates the search method used i.e. \code{geo} for geodesic search and \code{better} for \code{search_better}
#' Simulated data using different optimisation algorithms in guided tour.
#'
#'
#' @format
#' The code for simulating each data object is as follows:
#' \preformatted{
#' set.seed(123456);
#' holes_1d_geo <-
#'   animate_dist(boa5, tour_path = guided_tour(holes(), d = 1,
#'                                              search_f =  search_geodesic),
#'                rescale = FALSE)
#'
#' set.seed(123456)
#' holes_1d_better <-
#'   animate_dist(boa5, tour_path = guided_tour(holes(), d = 1,
#'                                              search_f =  search_better),
#'                rescale = FALSE)
#'
#'
#' set.seed(123456)
#' holes_2d_better <-
#'   animate_xy(boa6, tour_path = guided_tour(holes(), d = 2,
#'                                            search_f =  search_better),
#'             rescale = FALSE)
#'
#' set.seed(123456)
#' holes_2d_better_max_tries <-
#'   animate_xy(boa6, tour_path = guided_tour(holes(), d = 2,
#'                                            search_f = search_better,
#'                                            max.tries = 500),
#'              rescale = FALSE)
#'
#' library(tidyverse)
#' set.seed(1234)
#' x1 <- rnorm(1000, 0, 1)
#' x2 <- sample(c(rnorm(500, -3, 1), rnorm(500, 3, 1)), size = 1000)
#' x3 <- sample(c(rep(-1, 500), rep(1, 500)), size = 1000)
#' x4 <- sample(c(rnorm(250, -3, 1), rnorm(750, 3, 1)), size = 1000)
#' x5 <- sample(c(rnorm(330, -5, 1), rnorm(340, 0, 1), rnorm(330, 5, 1)), size = 1000)
#' x6 <- sample(c(rnorm(450, -5, 1), rnorm(100, 0, 1), rnorm(450, 5, 1)), size = 1000)
#' x7 <- sample(c(rnorm(500, -5, 1), rnorm(500, 5, 1)), size = 1000)
#' x8 <- rnorm(1000, 0, 1)
#' x9 <- rnorm(1000, 0, 1)
#' x10 <- rnorm(1000, 0, 1)
#'
#' boa <- tibble(x1 = x1, x2 = x2, x3 = x3, x4 = x4, x5 = x5,
#'               x6 = x6, x7 = x7, x8 = x8, x9 = x9, x10 = x10) %>%
#' boa <- as_tibble(scale(boa))
#' boa5 <- select(boa, x1, x2, x8: x10)
#' boa6 <- select(boa, x1, x2, x7: x10)
#' }
#'
#' @examples
#' library(ggplot2)
#' library(tidyr)
#' library(dplyr)
#' boa %>%
#'   pivot_longer(cols = x1:x10, names_to = "var", values_to = "value") %>%
#'   mutate(var = forcats::fct_relevel(as.factor(var), paste0("x", 1:10))) %>%
#'   ggplot(aes(x = value)) +
#'   geom_density() +
#'   facet_wrap(vars(var))
#' holes_1d_better %>% explore_trace_search() +
#'   scale_color_continuous_botanical(palette = "fern")
#' holes_2d_better_max_tries %>% explore_trace_search() +
#'   scale_color_continuous_botanical(palette = "daisy")
#' @rdname data
"holes_1d_geo"

#' @format
#' @rdname data
"holes_1d_better"

#' @format
#' @rdname data
"holes_2d_better"

#' @format
#' @rdname data
"holes_2d_better_max_tries"

#' @format
#' @rdname data
"boa"

#' @format
#' @rdname data
"boa5"

#' @format
#' @rdname data
"boa6"
